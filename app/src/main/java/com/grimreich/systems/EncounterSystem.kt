...
data class EncounterChoice(
    ...
    val combatEnemyType: EnemyType? = null // Added for cleaner combat handling
)
...
class EncounterSystem @Inject constructor(
    ...
    private val encounters = mutableListOf(
        ...
        Encounter(
            ...
            listOf(
                EncounterChoice("[Perception 12] Przeszukaj skrytkę", "Znalazłeś stare monety!", "perception", 12, effect = { state ->
                    state.gold += 50
                    "Znalazłeś 50 złota!"
                }),
        ...
        Encounter(
            ...
            EncounterType.INTERACTIVE,
            listOf(
                EncounterChoice("[Intelligence 14] Rozszyfruj działanie", "Ustabilizowałeś fragment rzeczywistości!", "intelligence", 14, effect = { state ->
        ...
        Encounter(
            ...
            EncounterType.INTERACTIVE,
            listOf(
                EncounterChoice("[Intelligence 14] Pomóż mu skatalogować otoczenie", "Pamięć jest kotwicą.", "intelligence", 14, effect = { s ->
                ...
                EncounterChoice("Zabierz księgę siłą [Strength 15]", "Księga rozpada się w proch.", "strength", 15, effect = { s ->
                    s.gold += 100
                    s.world.globalStability -= 5
        ...
        Encounter(
            ...
            EncounterType.INTERACTIVE,
            listOf(
                EncounterChoice("[Charisma 13] Uspokój dziecko", "Rzeczywistość odzyskuje ostrość.", "charisma", 13, effect = { s ->
                    s.party.forEach { h -> h.hp = (h.hp + 10).coerceAtMost(h.maxHp) }
                    "Dziewczynka uśmiecha się. Odzyskaliście spokój ducha."
                }),
                EncounterChoice("[Perception 15] Zbadaj niebo", "Widzisz błękitny kod.", "perception", 15, effect = { s ->
    ...
    fun rollEncounter(random: Random, state: GameState): Encounter? {
        ...
        if (hostileFactions.isNotEmpty() && random.nextFloat() < 0.2f) {
            ...
            val enemyType = when(factionId.uppercase()) {
                "CHURCH", "INKWIZYCJA" -> EnemyType.CITY_GUARD // fallback for pilot
            ...
            return Encounter(
                ...
                choices = listOf(
                    EncounterChoice("Walcz o życie!", "Rozpoczyna się brutalne starcie.", effect = { "POJEDYNEK" }, combatEnemyType = enemyType)
                )
            )
        }
