...
class LootSystemTest {
    ...
    fun `awardLootFromTableDirect awards gold correctly`() {
        ...
        assertEquals(110, state.gold) // 100 base + 10 reward
    }

    @Test
    fun `awardLootFromTableDirect awards items based on chance`() {
        val state = GameState()
        val sword = Item(id = "sword", name = "Miecz", type = "weapon", slot = "weapon")
        whenever(itemCatalogue.get("sword")).thenReturn(sword)
        ...
        assertEquals("sword", state.inventory[0].id)
