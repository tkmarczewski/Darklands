...
class VectorDraw:
    ...
    def rectangle(self, bounds, fill):
        ...
        x0, y0 = bounds[0]
        x1, y1 = bounds[1]
        self.paths.append(f'<path android:fillColor="{self._color(fill)}" android:pathData="M{x0},{y0} L{x1},{y0} L{x1},{y1} L{x0},{y1} Z" />')
    ...
    def line(self, points, fill, width=1):
        ...
        x0, y0 = points[0]
        x1, y1 = points[1]
        self.paths.append(f'<path android:strokeColor="{self._color(fill)}" android:strokeWidth="{width}" android:pathData="M{x0},{y0} L{x1},{y1}" />')

    def polygon(self, pts, fill):
        if fill is None: return
        d = "M" + " L".join([f"{p[0]},{p[1]}" for p in pts]) + " Z"
        self.paths.append(f'<path android:fillColor="{self._color(fill)}" android:pathData="{d}" />')

    def get_xml(self):
        header = '<vector xmlns:android="http://schemas.android.com/apk/res/android"\n    android:width="24dp"\n    android:height="24dp"\n    android:viewportWidth="64"\n    android:viewportHeight="64">\n'
        return header + "\n".join(["    " + p for p in self.paths]) + "\n</vector>"

# Copy drawing functions from generate_grimreich_icons.py
# (Actually I can just import them if I modify the original script to be modular, but I'll paste them for simplicity)
...
for fname in filenames:
    ...
    if res:
        ...
        try:
            fn(vd, (32, 32), col)
            with open(f"app/src/main/res/drawable/{fname}.xml", "w") as f:
                f.write(vd.get_xml())
            print(f"Generated Vector: {fname}.xml")
        except Exception as e:
            print(f"Error generating {fname}: {e}")

print("Done generating 64 vector icons.")
